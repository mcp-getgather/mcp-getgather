...
class BrandStateManager:
    ...
    async def _load_brand_states(self) -> list[BrandState]:
        """Load brand states from database."""
        brand_states_data = await db_manager.get("brand_states")
    ...
    async def _save_brand_states(self, brand_states: list[BrandState]) -> None:
        """Save brand states to database."""
        data = [brand_state.model_dump() for brand_state in brand_states]
        await db_manager.set("brand_states", data)

    async def get_by_brand_id(self, brand_id: BrandIdEnum) -> BrandState | None:
        """Get a brand state by its brand ID."""
        brand_states = await self._load_brand_states()
        for brand_state in brand_states:
            if brand_state.brand_id == brand_id:
                return brand_state
        return None

    async def update_is_connected(self, brand_id: BrandIdEnum, is_connected: bool) -> None:
        ...
        await self._save_brand_states(brand_states)

    async def is_brand_connected(self, brand_id: BrandIdEnum) -> bool:
        """Check if a brand is connected."""
        state = await self.get_by_brand_id(BrandIdEnum(brand_id))
        return state.is_connected if state else False

    async def get_browser_profile_id(self, brand_id: BrandIdEnum) -> str | None:
        """Get the browser profile ID for a brand."""
        state = await self.get_by_brand_id(BrandIdEnum(brand_id))
        return state.browser_profile_id if state else None

    async def add(self, brand_state: BrandState) -> None:

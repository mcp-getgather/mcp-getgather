...
from getgather.mcp.shared import auth, extract, start_browser_session
...
async def login(
    ...
    """Login to Ebird if needed"""

    request = ctx.get_http_request()
    headers = request.headers

    inputs["username"] = inputs.get("username",  headers.get(
        'EBIRD_USERNAME', settings.EBIRD_USERNAME))
    inputs["password"] = inputs.get("password", headers.get(
        'EBIRD_PASSWORD', settings.EBIRD_PASSWORD))
    inputs["submit"] = "true"

    return await auth(session_id=ctx.session_id, brand_id=BrandIdEnum("ebird"), inputs=inputs, current_page_spec_name=current_page_spec_name)
...
async def get_explore_species_list(
    ...
    spec_schema = SpecSchema.model_validate({
        ...
        "columns": [
            {
                "name": "species_name",
                "selector": "span.Suggestion-text"
            },
            {
                "name": "sci_name",
                "selector": "span.Suggestion-text span"
            },
        ],
    })
